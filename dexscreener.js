...
async function main() {
	...
	try {
		...
		for (let i = startPage; i <= maxPage; i++) {
			...
			console.log(`Extracted projects from page ${i}:`, projects);
		...
		for (let item of resultProject) {
					...
					.map((item, index) => {
						...
						if (boughtTotal) {
							...
							if (boughtTotal.endsWith("K")) {
								boughtValue =
									parseFloat(boughtTotal.replace("$", "").replace("K", "")) *
									1000;
							} else if (boughtTotal.endsWith("M")) {
								boughtValue =
									parseFloat(boughtTotal.replace("$", "").replace("M", "")) *
									1000000;
							} else {
						...
						if (pnl) {
							...
							if (pnl.endsWith("K")) {
								pnlValue =
									parseFloat(pnl.replace("$", "").replace("K", "")) * 1000;
							} else if (pnl.endsWith("M")) {
								pnlValue =
									parseFloat(pnl.replace("$", "").replace("M", "")) * 1000000;
							} else {
								pnlValue = parseFloat(
									pnl.replace("$", "").replace(/,/g, "").replace("$", "")
								);
							}
						}
						...
						const formattedBoughtTotal =
							boughtValue !== null ? formatNumber(boughtValue) : null;
						const formattedPnl =
							pnlValue !== null ? formatNumber(pnlValue) : null;

						const addressUrl = addresSelector
							? addresSelector.getAttribute("href")
							: null;
			...
			console.log(`Extracted data for project:`, pageData);
